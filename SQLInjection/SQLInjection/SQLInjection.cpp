...
std::string replace_substring(std::string s, const std::string s_to_replace, const std::string s_replace) {
    for (size_t position = 0; ; position += s_replace.length()) {

        position = s.find(s_to_replace, position);

        if (position == std::string::npos || s.empty()) break;
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Found an or command, this at best is a red flag
    // Obviously this would false positive flag anyone that used a username that contained 'OR' which isn't good enough
    if (sql.find("or") != std::string::npos) {
        std::string cleaned = sql.c_str();
        cleaned = replace_substring(cleaned, "\'", "\"");
        std::cout << "Detected 'OR' in query! Possible SQL Injection!" << std::endl;
        std::cout << "Stopped Query: " << sql.c_str() << std::endl;
        std::cout << "Escaped/Cleaned version of query: " << cleaned << std::endl;
        return false;
    }
